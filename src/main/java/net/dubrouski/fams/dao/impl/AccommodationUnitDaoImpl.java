...
@Stateless
@Named(value = "unitDao")
public class AccommodationUnitDaoImpl extends
		BaseDaoImpl<AccommodationUnit, Long> implements AccommodationUnitDao {
	...
	public AccommodationUnit getParent(AccommodationUnit unit) {
		...
		Query q = entityManager
				.createQuery("SELECT u FROM AccommodationUnit u JOIN u.children p WHERE p.id = :id");
	...
	@Override
	public void removeChild(AccommodationComposite parent,
	...
	// When more composite elements are added in the future, Address will need
	// refactoring.
		...
		if (!(unit instanceof Place)) {
			if (((AccommodationComposite) unit).childrenCount() > 0) {
				throw new FmsException(
						"Cannot change address if unit has children.");
			}
		}
		if (getParent(unit) != null) {
	...
	private void setAddressForChildren(AccommodationUnit unit, Address address) {
		if ((unit instanceof Place)
				|| (((AccommodationComposite) unit).childrenCount() == 0)) {
			unit.setAddress(address);
			update(unit);
		} else {
	...
	public List<AccommodationUnit> listAccommodationsByType(String type) {
		if (type.equals("room") || type.equals("place")) {
			Query q = entityManager
					.createQuery("SELECT u FROM AccommodationUnit u WHERE u.type = :type");
	...
	public List<AccommodationUnit> getAccommodationsByPage(int pageSize,
		...
		TypedQuery<AccommodationUnit> query = this.entityManager
				.createQuery("select p from AccommodationUnit p",
						AccommodationUnit.class).setFirstResult(first)
